...
This is the MolmoAct2 release repo, but in this working directory only a handful of scripts are actively maintained:

- `host_server.py` — FastAPI inference server that loads `allenai/MolmoAct2-DROID` and exposes `/act`.
- `host_server_yam.py` — same shape, but for `allenai/MolmoAct2-BimanualYAM`: 3 cameras `[top, left, right]`, 14-D state, `norm_tag="yam_dual_molmoact2"`.
- `inference_script.py` — client bridge that talks to the DROID server from a Polymetis NUC driving a Franka.
...
uv run python host_server.py --host 0.0.0.0 --port 8000            # DROID server (bf16 default)
uv run python host_server_yam.py --host 0.0.0.0 --port 8202        # YAM server (bf16 default; YAM default port is 8202 to coexist with DROID on 8000)
uv run python host_server.py --dtype float32 --cuda-graph          # full precision + CUDA graphs
uv run hf download allenai/MolmoAct2-DROID                         # pre-cache DROID (~22 GB)
uv run hf download allenai/MolmoAct2-BimanualYAM                   # pre-cache YAM   (~22 GB)
curl http://<host>:8000/act                                        # health check
```
...
`inference_script.py` is the DROID reference client. There's no YAM bridge in this repo yet; if you write one, mirror the YAM schema above. The bridge is hand-rolled HTTP (`requests.post`), not auto-generated, so server-schema changes need matching client edits.

## Server architecture — non-obvious bits

The MolmoAct2 checkpoints were not released with `bfloat16` or local-snapshot loading in mind. Both `host_server.py` and `host_server_yam.py` apply the same set of upstream workarounds; future changes need to preserve these or a server will silently break:
...
- Each `host_server*.py` is the source of truth for its embodiment's `/act` schema, `NORM_TAG`, and state/camera count. New deployments (SO-100/101, LIBERO) should clone the same template and override those constants — don't try to multiplex embodiments behind one server.
- `predict_action`'s mode kwarg differs across checkpoints: the DROID revision uses `action_mode="continuous"` (defaulted), the YAM revision uses `inference_action_mode="continuous"` (required, no default). Each `host_server*.py` must use its checkpoint's name — copying the call site verbatim between embodiments will TypeError at warmup.
...
- Keep `inference_script.py` runnable on the Polymetis NUC: it depends on `pyzed.sl`, `cv2`, `requests`, `json_numpy`, and optionally `pandas` for Excel logging. Don't introduce server-only deps into it.
