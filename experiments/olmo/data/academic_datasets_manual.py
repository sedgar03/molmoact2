...
from olmo.util import compute_hash, resource_path
...
if DATA_HOME is not None:
    ...
    MULSET_SOURCE = join(DATA_HOME, "MulSeT")
else:
    CHARTQA_SOURCE = None
    DOCQA_SOURCE = None
    INFOQA_SOURCE = None
...
class ScienceQAImageOnly(Dataset):
    ...
    def download(self, n_procs=1):
        ...
        for split in ["train", "validation", "test"]:
            ...
            write_json(join(DATA_HOME, "science_qa_img_only", f"{split}.json"), examples)
...
class MulSet(DatasetBase):
    """2 images per question"""
    PATH = "WanyueZhang/MulSeT"
    ...
    def download(cls, n_procs=1):
        ...
        logging.info(f"Downloading MulSeT...")
        os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "1"
        snapshot_download(
            repo_id=cls.PATH,
            repo_type="dataset",
            revision="main",
            local_dir=join(DATA_HOME, "MulSeT"),
    ...
    def load(self):
        split = self.split
        data = []
        for task in ["mask", "distance", "direction"]:
            src_path = join(MULSET_SOURCE, split, f"merged_{task}_questions.jsonl")
            with open(resource_path(src_path), "r") as f:
                for line in f.readlines():
                    ex: dict = json.loads(line)
                    ex = dict(**ex, task=task)
                    data.append(ex)
        return data
    ...
    def get(self, item, rng):
        ...
        if ex["task"] == "mask":
            images = [
                join(MULSET_SOURCE, split, ex["image_1"]),
                join(MULSET_SOURCE, split, ex["mask_image"]),
            ]
            options = ex["options"]
        else:
            images = [
                join(MULSET_SOURCE, split, ex["image_1"]),
                join(MULSET_SOURCE, split, ex["image_2"]),
            ]
            options = [opt["text"] for opt in ex["options"]]
        
        out = dict(
            image=images,
            question=ex["query"],
            answer=ex["answer"],
            style=f"eval_multi_image_mc",
            metadata=dict(
                task=ex["task"],
                options=options,
            )
