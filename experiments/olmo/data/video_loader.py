    ...
    def __call__(self, video_path: str, clip: Optional[Tuple[float, float]] = None,
        ...
        if self.loading_method == "torchcodec_exact":
            try:
                ...
                log.warning(
                    "TorchCodec exact decode failed for %s (%s: %s); falling back to av_noseek.",
                    video_path,
                    type(e).__name__,
                    e,
                )
            ...
            try:
                ...
                log.warning(
                    "TorchCodec approximate decode failed for %s (%s: %s); falling back to av_noseek.",
                    video_path,
                    type(e).__name__,
                    e,
                )
        ...
        if fake_timestamp_fps:
            # Some tracking/counting datasets provide annotation timestamps on a synthetic
            # fixed-fps timeline (for example 2fps) while we may sample frames sparsely
            # across a much longer clip (e.g. uniform_last_frame over 30s). In that case,
            # the raw sampled timestamps are not expected to numerically match the fake fps.
            # Build a synthetic timeline from frame order so the visual tokens and
            # annotation timestamps live in the same fake-fps space.
            fake_timestamps = np.arange(len(timestamps), dtype=np.float64) / float(fake_timestamp_fps)
            return VideoFrames(
