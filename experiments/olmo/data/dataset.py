...
class DeterministicDataset:
    ...
    def __init__(
        ...
        self.dataset_name = dataset_name or getattr(dataset, "dataset_name", None) or getattr(dataset, "name", None)
    ...
    def get(self, idx, epoch=0):
        ...
        if idx >= len(self.dataset):
            # Padding example
            item = dict(self.dataset.get(0, rng))
        ...
        if metadata is None:
            ...
            metadata = None
        if metadata is not None and self.dataset_name is not None and "dataset_name" not in metadata:
