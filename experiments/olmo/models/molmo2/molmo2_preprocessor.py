...
from olmo.util import get_hf_access_token, interpolate_frame_scores
log = logging.getLogger(__name__)


@dataclass
class Molmo2PreprocessorConfig(VideoLoaderConfig, TextPreprocessorConfig):
    ...
    def build(self, tokenizer, image_preprocessor, text_seq_len=None, max_sequence_length=None):
        if self.image_padding_mask:
            raise NotImplementedError("Image padding mask is not implemented for Molmo2.")
...
class VideoPreprocessor:
    ...
    def __call__(
        ...
        if self.query_based_resolution_selection:
            ...
            # 1 frame example, '/weka/oe-training-default/mm-olmo/video_datasets/ActivityNet/all-videos/-3baQGb2zIY/-3baQGb2zIY.mp4' -> {'clip_start_time': 0.01, 'clip_end_time': 0.02}
